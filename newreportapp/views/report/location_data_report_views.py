...
def location_data_view(request, id=None, header_report_id=None):
    subject = "location_data"
    ...
    Utilize para indicar a localização, como endereço ou coordenadas geográficas."""
    ...
    if request.method == "POST":
        ...
        # Atualiza manualmente os campos que podem não estar vindo corretamente no POST
        post_data = request.POST.copy()
        post_data["title"] = title  # Garante que o título seja enviado para o formulário
        post_data["order"] = report_order
        form = SectionReportForm(post_data, instance=report_section)

        if form.is_valid():
            section_report = form.save(commit=False)

            # Pega a descrição do POST corretamente
            section_report.description = request.POST.get("data_location_description", "")
            section_report.order = report_order
    ...
    return render(request, 'report/location_data_report.html', {
